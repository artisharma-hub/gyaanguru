  ...
  void _onTabChanged() {
    ...
    switch (_tabController.index) {
      ...
      case 1:
        ...
        break;
      case 2:
        ref.read(leaderboardProvider.notifier).fetchCategory(_selectedCategory);
        break;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BackButtonListener(
        ...
        child: Scaffold(
          extendBody: true,
          bottomNavigationBar:
              AppNavBar(currentIndex: _navIndex, onTap: _onNavTap),
          body: Column(
            children: [
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        if (showMyEntry && idx == rest.length + 1) {
                          return _PlayerRow(
                              player: myEntry, rank: myRank, isMe: true);
                        }
                        final p = rest[idx];
...
class _CategoryDropdown extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DropdownButtonFormField<String>(
      ...
      items: categories
          .map((c) =>
              DropdownMenuItem(value: c, child: Text(categoryNames[c] ?? c)))
          .toList(),
      onChanged: onChanged,
    );
  }
      ...
      child: Row(
        ...
        children: [
          if (second != null)
            Expanded(
                child: _PodiumItem(
                    player: second, rank: 2, avatarRadius: 28, nameSize: 12)),
          ...
          if (third != null)
            Expanded(
                child: _PodiumItem(
                    player: third, rank: 3, avatarRadius: 26, nameSize: 12)),
        ],
      ),
    );
  }
...
class _PodiumItem extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: AvatarWidget(
              name: name, avatarColor: avatarColor, radius: avatarRadius),
        ),
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border:
                Border.all(color: rankColor.withValues(alpha: 0.40), width: 1),
          ),
          child: Center(
            child: Text(
              '$rank',
  ...
  String _formatScore(int s) {
    if (s >= 1000)
      return '${(s / 1000).toStringAsFixed(3).replaceAll('.', ',')}';
    return s.toString();
  }
}
...
class _PlayerRow extends StatelessWidget {
  ...
  final bool isMe;
  const _PlayerRow(
      {required this.player, required this.rank, required this.isMe});
